...
replacement when `shopt -s patsub_replacement` (Bash >= 5.2) is enabled (which
is the default of Bash >= 5.2).

For example, the array expansions with a pattern replacement may be used to add
a prefix to every element in an array:
...
However, this has the problem. The characters `&` contained in `$prefix`, if
any, will be replaced with the matched string. The unexpected
`patsub_replacement` may be suppressed by quoting the replacement as
...
However, this has another problem in bash < 4.3 or when `shopt -s compat42` is
turned on. The inner double quotations are treated literally so that the
`PREFIX` instead of ``"PREFIX"` is prefixed to elements. To avoid this
situation, the outer double quotations might be removed, but this has even
another problem of the pathname expansions and `IFS`.
...
In a general case, one needs to modify each array element in a loop, where only
the replacement is quoted.
